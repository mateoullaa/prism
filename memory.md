...
- [2026-06-22] observables structure: each IOC gets type + value + is_public + verdict (from enrichment only, independent of alert verdict) + confidence + sources (only ok/cached providers) + reasons. Verdict derivation: ≥2 strong signals → malicious/95, 1 strong → malicious/75, weak only → suspicious/40, none/error → unknown/0. Tested end-to-end: observable.verdict="malicious" while alert.verdict="NEEDS_REVIEW" confirms independence.
...
- [2026-06-26] MITRE mapping fixed via Python pre-evaluation: _evaluate_mitre() maps alert_type → ATT&CK technique deterministically (same design principle as _evaluate_enrichment). Prompt simplified from 6-line rule table to 2 lines referencing the pre-computed result. LLM receives "MITRE mapping (pre-evaluated): T1110 Brute Force — include this in your output as-is." 269 tests passing.
- [2026-06-26] risk_score determinism fixed: BLAS floating-point non-determinism causes qwen2.5:3b to drift (e.g. FP: 1↔2, TP: 7↔8) even at temperature=0. Fix: verdict-range enforcement in Python after LLM coercion — FP→1 (always), TP→max(8,min(10,score)), NR→unchanged. Same design principle: Python makes deterministic calls, LLM only provides input. 272 tests passing.
- [2026-06-26] Apache extension to parser: alert_type="apache" via decoder.name="apache-errorlog"; _extract_apache() extracts data.srcip + GeoLocation. Purely additive (0 deletions). RFC-5737 IPs (203.0.113.x, 198.51.100.x) are treated as private in Python 3.11+ — use real routable IPs in fixtures (e.g. 5.188.206.55).
- [2026-06-30] v2.2 RAG COMPLETE & SHADOW-DEPLOYED: ChromaDB (in-process PersistentClient, ./chroma_db bind-mount); embeddings via Ollama /api/embeddings + nomic-embed-text; two functions: (1) auto-FP via similarity ≥AUTO_FP_THRESHOLD (gated RAG_SHADOW_MODE=true, no-op by default), (2) context enrichment (live, injected into reasoner). Feedback-loop guard: only real LLM verdicts (reasoner_meta status==ok) indexed back. All tools/retriever.py fail-safe (never raises). Backfill + validation scripts provided. Deploy-time: pull nomic-embed-text, backfill_chroma.py, validate_threshold.py, then RAG_ENABLED=true (keep shadow first, flip after precision confirmed). 309 tests passing.
- [2026-07-01] v2.2 LIVE in production (shadow mode): backfill_chroma.py indexed 31 alerts (18 TP, 9 NR, 4 FP). Context injection (Function 2) is live; auto-FP (Function 1) remains shadow until corpus grows. Learning: 31-alert corpus too small for safe auto-FP activation. Target ≥200 alerts before validate_threshold.py + flip RAG_SHADOW_MODE=false. Corpus grows with each new alert.
- [2026-07-01] v2.2 design decisions (RAG): (1) Ollama nomic-embed-text for embeddings (no ML deps added to image); (2) ChromaDB in-process PersistentClient at ./chroma_db (bind-mounted, offline-safe); (3) shadow-first: context injection live immediately, auto-FP waits corpus ≥200; (4) no CSV schema change — auto_fp traced via status="auto_fp"/model="rag-similarity:nomic-embed-text"/reason field; (5) full_description = case_description + correlation_summary (English, analyst-facing); (6) correlation_summary is None when RAG disabled or no hits (backward compatible).

## Resolved errors
- [2026-06-26] Two bugs found in live validation and fixed: (1) _evaluate_mitre() did not check is_known_fp_candidate — added guard (return None if FP candidate); (2) FP guardrail left risk_score=1 after downgrading to NEEDS_REVIEW — added reset to 5 in guardrail block. 275 tests passing.
...
- [2026-06-26] Docker deployment: Prism corre en contenedor Docker en 192.168.11.105:8000. Ollama (fuera de Docker) alcanzable via extra_hosts host-gateway:host-gateway → http://host-gateway:11434. Volúmenes: metrics/ (CSV audit log) y config/ (known_patterns.json). Comandos clave: "docker compose up -d" (levantar), "docker compose down" (parar), "git pull && docker compose up -d --build" (actualizar). python main.py ya no se usa — Docker maneja el entrypoint.
